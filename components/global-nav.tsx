...
import { Droplets, Home, MessageCircle, Heart, User, LogOut } from 'lucide-react'
import { useAuth } from '@/app/providers'

export function GlobalNav() {
  const { userEmail, authReady, signOut } = useAuth()
        ...
        <div className="flex items-center gap-5">
          ...
          {!authReady ? (
            ...
            <div className="flex items-center gap-3 border-l border-border pl-5">
              ...
              <Link
                href="/profile"
                className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium text-foreground transition-colors hover:bg-muted"
              >
                <User size={16} />
                Profile
              </Link>
